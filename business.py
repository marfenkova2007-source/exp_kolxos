...
def _apply_economy_scoring(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df_scored = df.copy()
    df_scored['score'] = (
        (np.log1p(df_scored['mentions']) * 10) + 
        df_scored['rarity_score'] + 
        (df_scored['is_premium'] * 5)
    )
...
def _apply_inspiration_scoring(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df_scored = df.copy()
    premium_multiplier = df_scored['is_premium'].map({1: 1.5, 0: 1.0})
    df_scored['score'] = (df_scored['rarity_score'] * 2) * premium_multiplier
    return df_scored.sort_values(by='score', ascending=False)
...
def get_menu_stats(menu_json_path: str) -> tuple[Counter, int, str]:
    ...
    with open(menu_json_path, 'r', encoding='utf-8') as f:
        data = json.load(f)
    cuisine_type = data.get('cuisine_type', 'Авторская')
...
def get_seasonal_df(start_month: int, end_month: int, db_path: str = 'farm_data.db') -> pd.DataFrame:
...
def _calculate_status(row: pd.Series) -> str:
    """Определяет категорию товара: base, rare или premium"""
    if row['is_premium'] == 1:
        return "premium"
    elif row['rarity_score'] >= 10:
        return "rare"
    else:
        return "base"
    
def _extract_intersection_months(row: pd.Series, target_months: list[int]) -> list[int]:
...
def get_general_recommendations(start_month: int, end_month: int, db_path: str = 'farm_data.db') -> dict:
    ...
    df = df.drop_duplicates(subset=['main_ingredient', 'shop_name']).head(15)
...
def get_menu_recommendations(menu_json_path: str, start_month: int, end_month: int, db_path: str = 'farm_data.db') -> dict:
    ...
    df['novelty_score'] = 100.0 - (df['mentions'] / total_dishes * 100.0 if total_dishes > 0 else 0)

    df_economy = df[df['mentions'] > 0].copy()
    
    if not df_economy.empty:
        df_economy = _apply_economy_scoring(df_economy)
        df_economy = df_economy.drop_duplicates(subset=['main_ingredient', 'shop_name']).head(7)

    df_new = df[df['mentions'] == 0].copy()
    
    if not df_new.empty:
        df_new = _apply_inspiration_scoring(df_new)
        df_new = df_new.drop_duplicates(subset=['main_ingredient', 'shop_name']).head(8)

    final_df = pd.concat([df_economy, df_new])
    result_list = []
    for _, row in final_df.iterrows():
        reason_type = "economy" if row['mentions'] > 0 else "inspiration"
        ...
        result_list.append({
            ...
            "link": row['url_product'],
            "status": _calculate_status(row),
            "novelty_score": round(row['novelty_score'], 1),
            "season_months": _extract_intersection_months(row, target_months),            
