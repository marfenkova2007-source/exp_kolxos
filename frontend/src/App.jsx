...
export default function App() {
  ...
  // НОВОЕ: состояние для определения режима (анализ меню или просто просмотр)
  ...
  useEffect(() => {
    if (step === 'loading') {
      ...
      document.title = isAnalysisMode ? "Рекомендации по меню" : "Сезонный каталог";
    } else {
      document.title = "Сезонные продукты";
    }
  }, [step, isAnalysisMode]);
  ...
  const filteredProducts = data?.products?.filter(p => {
    const matchTab = isAnalysisMode ? p.recommendation_type === activeTab : true;
            ...
            <div className="max-w-6xl mx-auto flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div>
                <h2 className="text-xl font-bold">
                  {isAnalysisMode ? "Рекомендации для Вас" : "Сезонный календарь"}
                </h2>
